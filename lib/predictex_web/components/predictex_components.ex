defmodule PredictexWeb.PredictexComponents do
  ...
  def brand_logo(assigns) do
    ...
    <svg width={@size} height={@size} viewBox="0 0 36 36" class={@class} aria-hidden="true">
      ...
      <circle cx="18" cy="18" r="10" fill="none" stroke="currentColor" stroke-width="2.6" class="text-accent" />
    ...
    <div class={[
      ...
      <div :if={@pred && @done?} class="mt-3 flex flex-wrap items-center justify-center gap-2">
        <span class="text-[11px] text-base-content/60">
          Actual <strong class="font-score text-base-content">{@fx.fixture.home_goals}–{@fx.fixture.away_goals}</strong>
        </span>
        <span
          :if={@fx.exact?}
  ...
  def auth_card(assigns) do
    ...
    <div class="mx-auto max-w-4xl overflow-hidden rounded-box border border-base-300 bg-base-100 shadow-xl lg:grid lg:grid-cols-2">
      ...
      <div class="relative hidden overflow-hidden bg-gradient-to-br from-primary to-secondary p-8 text-white lg:flex lg:flex-col lg:justify-between">
        <div class="pointer-events-none absolute -right-12 -top-10 size-52 rounded-full border-2 border-accent/25"></div>
        <div class="pointer-events-none absolute right-2 top-5 size-32 rounded-full border-2 border-white/10"></div>
  ...
  def scoring_legend(assigns) do
    ...
    <div class="rounded-box border border-base-300 bg-base-100 p-4">
      ...
      <p class="mt-3 text-[11px] leading-relaxed text-base-content/55">
        ...
        of players agreed. A booster <strong class="text-accent">2×</strong> doubles one fixture per round.
      </p>
    </div>
    """
  end
  ...
  defp legend_chip(assigns) do
    ~H"""
    <span class={["inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-xs font-semibold", chip_tone(@tone)]}>
  ...
  defp row_accent(_rank, true), do: "border-primary/50 border-l-[3px] border-l-primary bg-primary/10"
  defp row_accent(1, _you), do: "border-accent/40 border-l-[3px] border-l-accent bg-accent/10"
  defp row_accent(2, _you), do: "border-base-content/20 border-l-[3px] border-l-base-content/40 bg-base-content/5"
  defp row_accent(3, _you), do: "border-warning/25 border-l-[3px] border-l-warning/60 bg-warning/5"
  defp row_accent(_rank, _you), do: "border-base-300 bg-base-100"

  defp card_border(%{exact?: true, status: :completed}, _no_pick), do: "border border-accent/45 ring-1 ring-accent/15"
  defp card_border(_fx, true), do: "border border-dashed border-error/50"
  defp card_border(_fx, _no_pick), do: "border border-base-300"

  defp score_box(%{exact?: true, status: :completed}, _no_pick), do: "border-accent/40 bg-base-300 text-accent"
